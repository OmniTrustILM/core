...
-- The connector's opaque run handle, created as run_meta by the v2 schema groundwork. Renamed so it cannot be read as
-- metadata; the contract renamed the same field from meta to checkpoint for the same reason.
...
--   applied. Two status calls for one run can be in flight at once -- a call slower than the claim floor is
--   published again -- and nothing else orders their answers, so an older one landing second would overwrite newer
--   counters and stamp them as freshly recorded. The connector promises this value never decreases, which makes it
--   the one field that can tell the two apart.
ALTER TABLE "discovery"
