...
public abstract class AbstractSigningRecordWriter implements SigningRecordWriter {
    ...
    protected SigningRecord buildSigningRecord(SigningRecordInput input) {
        ...
        if (v.isRecordRequestMetadata()) r.setRequestMetadataJson(input.getRequestMetadataJson());
        if (v.isRecordSignature())       r.setSignatureValue(input.getSignature());
        if (v.isRecordSignedDocument())  r.setSignedDocument(input.getSignedDocument());
        if (v.isRecordDtbs())            r.setDtbs(input.getDtbs());
        return r;
    }
}
