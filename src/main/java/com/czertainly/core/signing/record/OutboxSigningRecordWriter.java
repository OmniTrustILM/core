    ...
    public void record(SigningRecordInput input) {
        ...
        timed("DEFERRED_DURABLE", () -> {
            ...
            if (v.isRecordRequestMetadata())
                row.setRequestMetadataJson(input.getRequestMetadataJson());
            if (v.isRecordSignature())
                row.setSignatureValue(input.getSignature());
            if (v.isRecordSignedDocument())
                row.setSignedDocument(input.getSignedDocument());
            if (v.isRecordDtbs())
                row.setDtbs(input.getDtbs());
            repository.save(row);
