...
public class DiscoveryProviderV2Adapter implements DiscoveryProviderAdapter {
    ...
    /**
     * The contract's cap on a serialized run handle. Enforced on every checkpoint-returning response, because the
     * handle is replayed on every later call: accepting an oversized one would produce a run whose every tick sends a
     * request the transport cannot carry.
     */
    ...
    private boolean recordInitiated(UUID discoveryUuid, DiscoveryInitiateResponseDto response,
        ...
        return Boolean.TRUE.equals(transactionHandler.runInNewTransaction(() -> {
            ...
            enforceMetaCap(response.getCheckpoint());
     ...
     */
    private void dropAtConnector(Discovery run, List<MetadataAttribute> checkpoint) {
        try {
    ...
    private static void enforceMetaCap(List<MetadataAttribute> checkpoint) {
        int size = serializedSize(checkpoint);
        if (size > MAX_META_BYTES) {
            throw new RunRefusedException("checkpoint size exceeded: " + size + " bytes, cap " + MAX_META_BYTES);
        }
    }

    private static boolean exceedsMetaCap(List<MetadataAttribute> checkpoint) {
        return serializedSize(checkpoint) > MAX_META_BYTES;
    }

    /** Serialized size of an attribute list, the measure both the checkpoint cap and the run metadata cap use. */
    static int serializedSize(List<MetadataAttribute> attributes) {
        if (attributes == null || attributes.isEmpty()) {
            return 0;
        }
        String serialized = AttributeDefinitionUtils.serialize(attributes);
        return serialized == null ? 0 : serialized.getBytes(StandardCharsets.UTF_8).length;
    }

    /**
     * Ends a run whose connector answered with a checkpoint too large to replay. Rolling the local write back would
     * leave Core describing a state the connector has left, and no tick reconciles that.
     */
    private void endUndriveable(Discovery run, String operation, List<MetadataAttribute> checkpoint) {
        logger
                .error("Discovery {} answered {} with a checkpoint of {} bytes, over the {} byte cap; ending the run",
                        run.getUuid(), operation, serializedSize(checkpoint), MAX_META_BYTES);
        terminator
        ...
        dropAtConnector(run, checkpoint);
    }
     ...
     */
    // Package-private so the test can assert the rule directly: through start(), every failure a stub can provoke
    // arrives as a ConnectorException.
    static String startFailureReason(Exception e) {
        // Not safeMessage for a ConnectorException: it is a PlatformException, so its connector-written prose would
        // pass the gate.
    ...
    public void stop(Discovery discovery) throws ConnectorException {
        ...
        if (exceedsMetaCap(response.getCheckpoint())) {
            endUndriveable(discovery, "stop", response.getCheckpoint());
            return;
        }
    ...
    public void resume(Discovery discovery) throws ConnectorException {
        ...
        if (exceedsMetaCap(response.getCheckpoint())) {
            endUndriveable(discovery, "resume", response.getCheckpoint());
            return;
        }
        transactionHandler.runInNewTransaction(() -> {
            Discovery locked = lock(discoveryUuid);
