...
public interface ResourceExtensionService {
     ...
     * MUST carry an object-scoped {@code @ExternalAuthorization(<KIND>, DETAIL)} taking the {@link SecuredUUID}
     * so the authorization aspect resolves the concrete object and fails closed when the calling user lacks
     * {@code DETAIL} on that specific object. This is the load-bearing per-object boundary the reference
     * expander's callback mode resolves through.
     * <p>
     ...
     *
     * @param objectUuid object to load, wrapped so the auth aspect can scope the DETAIL check
